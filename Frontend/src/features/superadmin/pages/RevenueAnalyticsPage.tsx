...
export default function RevenueAnalyticsPage() {
 ...
 <div className="bg-card rounded-xl border border-border p-5"><p className="text-xs text-muted-foreground">Month Change</p><p className={`text-2xl font-bold mt-1 flex items-center gap-1 ${change >= 0 ? 'text-foreground' : 'text-foreground'}`}>{change >= 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}{change}%</p></div>
