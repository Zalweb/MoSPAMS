    ...
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-6">
        <section className="bg-card rounded-2xl border border-border p-4">
          ...
          <div className="space-y-2.5">
            <Input placeholder="Plan code" value={planForm.planCode} onChange={(e) => setPlanForm(p => ({ ...p, planCode: e.target.value }))} className="bg-muted border-zinc-700 text-zinc-200 placeholder:text-muted-foreground" />
            <Input placeholder="Plan name" value={planForm.planName} onChange={(e) => setPlanForm(p => ({ ...p, planName: e.target.value }))} className="bg-muted border-zinc-700 text-zinc-200 placeholder:text-muted-foreground" />
            <Input type="number" placeholder="Monthly price" value={planForm.monthlyPrice} onChange={(e) => setPlanForm(p => ({ ...p, monthlyPrice: e.target.value }))} className="bg-muted border-zinc-700 text-zinc-200 placeholder:text-muted-foreground" />
        ...
        <section className="bg-card rounded-2xl border border-border p-4">
          ...
          <div className="space-y-2.5">
            <select className="w-full h-9 rounded-xl border border-zinc-700 bg-muted text-zinc-200 px-3 text-[12px]" value={subscriptionForm.shopId} onChange={(e) => setSubscriptionForm(p => ({ ...p, shopId: e.target.value }))}><option value="">Select shop</option>{shops.map(s => <option key={s.shopId} value={s.shopId}>{s.shopName}</option>)}</select>
            <select className="w-full h-9 rounded-xl border border-zinc-700 bg-muted text-zinc-200 px-3 text-[12px]" value={subscriptionForm.planId} onChange={(e) => setSubscriptionForm(p => ({ ...p, planId: e.target.value }))}><option value="">Select plan</option>{plans.map(p => <option key={p.planId} value={p.planId}>{p.planName}</option>)}</select>
            <select className="w-full h-9 rounded-xl border border-zinc-700 bg-muted text-zinc-200 px-3 text-[12px]" value={subscriptionForm.status} onChange={(e) => setSubscriptionForm(p => ({ ...p, status: e.target.value as typeof p.status }))}><option value="PENDING">PENDING</option><option value="ACTIVE">ACTIVE</option><option value="EXPIRED">EXPIRED</option><option value="CANCELLED">CANCELLED</option></select>
            <Input type="date" value={subscriptionForm.endsAt} onChange={(e) => setSubscriptionForm(p => ({ ...p, endsAt: e.target.value }))} className="bg-muted border-zinc-700 text-zinc-200" />
        ...
        <section className="bg-card rounded-2xl border border-border p-4">
          ...
          <div className="space-y-2.5">
            <select className="w-full h-9 rounded-xl border border-zinc-700 bg-muted text-zinc-200 px-3 text-[12px]" value={paymentForm.shopSubscriptionId} onChange={(e) => setPaymentForm(p => ({ ...p, shopSubscriptionId: e.target.value }))}><option value="">Select subscription</option>{subscriptions.map(s => <option key={s.shopSubscriptionId} value={s.shopSubscriptionId}>{s.shopName} | {s.planName}</option>)}</select>
            <Input type="number" placeholder="Amount" value={paymentForm.amount} onChange={(e) => setPaymentForm(p => ({ ...p, amount: e.target.value }))} className="bg-muted border-zinc-700 text-zinc-200 placeholder:text-muted-foreground" />
            <select className="w-full h-9 rounded-xl border border-zinc-700 bg-muted text-zinc-200 px-3 text-[12px]" value={paymentForm.paymentStatus} onChange={(e) => setPaymentForm(p => ({ ...p, paymentStatus: e.target.value as typeof p.paymentStatus }))}><option value="PAID">PAID</option><option value="PENDING">PENDING</option><option value="FAILED">FAILED</option><option value="REFUNDED">REFUNDED</option></select>
            <Input placeholder="Payment method" value={paymentForm.paymentMethod} onChange={(e) => setPaymentForm(p => ({ ...p, paymentMethod: e.target.value }))} className="bg-muted border-zinc-700 text-zinc-200 placeholder:text-muted-foreground" />
            <Input type="date" value={paymentForm.dueAt} onChange={(e) => setPaymentForm(p => ({ ...p, dueAt: e.target.value }))} className="bg-muted border-zinc-700 text-zinc-200" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <section className="bg-card rounded-2xl border border-border p-4 overflow-x-auto">
          ...
          <table className="w-full text-[12px] text-zinc-300 min-w-[360px]"><thead><tr className="border-b border-border"><th className="text-left py-2 text-muted-foreground">Shop</th><th className="text-left py-2 text-muted-foreground">Plan</th><th className="text-left py-2 text-muted-foreground">Days Left</th></tr></thead>
          <tbody className="divide-y divide-zinc-800">
            ...
            {subscriptions.map(sub => (
              editingSubId === sub.shopSubscriptionId ? (
                <tr key={sub.shopSubscriptionId} className="bg-muted/50">
                  ...
                  <td className="py-2 text-right"><button onClick={() => void saveEditSub()} className="text-emerald-400 hover:text-emerald-300 text-[11px] font-semibold mr-2">Save</button><button onClick={cancelEditSub} className="text-muted-foreground hover:text-zinc-300 text-[11px]">Cancel</button></td>
                </tr>
              ) : (
