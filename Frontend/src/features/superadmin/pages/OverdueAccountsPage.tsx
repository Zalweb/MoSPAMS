...
export default function OverdueAccountsPage() {
 ...
 <Calendar className="w-16 h-16 mx-auto mb-4 text-muted-foreground dark:text-zinc-600" /><h3 className="text-lg font-semibold text-foreground mb-2">No Overdue Accounts</h3><p className="text-muted-foreground">All subscriptions are current.</p>
 </div>
 ) : (
 ...
 <td className="px-6 py-4 text-muted-foreground dark:text-zinc-300">{a.planName}</td>
 <td className="px-6 py-4 text-foreground font-semibold">{CURRENCY_PREFIX}{a.monthlyPrice?.toLocaleString()}</td>
 <td className="px-6 py-4"><span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-foreground/10 text-foreground border border-foreground/20">{a.status}</span></td>
 <td className="px-6 py-4 text-muted-foreground">{a.endsAt ? new Date(a.endsAt).toLocaleDateString() : '-'}</td>
 <td className="px-6 py-4 text-right"><span className="inline-flex items-center gap-1 text-foreground font-semibold"><AlertTriangle className="w-4 h-4" />{a.daysOverdue}d</span></td>
 </tr>
 ))}</tbody></table>
 </div>
 </div>
