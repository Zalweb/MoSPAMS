...
function ChartSection({ title, data, dataKey }: any) {
  if (!data?.length) return <div className="bg-card rounded-2xl border border-border p-6"><h2 className="text-[16px] font-bold text-foreground mb-4">{title}</h2><p className="text-muted-foreground text-sm text-center py-12">No data available</p></div>;
  const maxVal = Math.max(...data.map((d: any) => d[dataKey]), 1);
  return (
    <div className="bg-card rounded-2xl border border-border p-6"><h2 className="text-[16px] font-bold text-foreground mb-4">{title}</h2>
